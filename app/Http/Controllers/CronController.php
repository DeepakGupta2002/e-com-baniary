...
class CronController extends Controller
{
    ...
    { 
        ...
        return DB::transaction(function () {
            $generalSetting = GeneralSetting::lockForUpdate()->firstOrFail();
            ...
            $eligibleUsers = UserExtra::where('bv_left', '>', 0)->where('bv_right', '>', 0)->lockForUpdate()->get();
            foreach ($eligibleUsers as $uex) {
                $weak = min($uex->bv_left, $uex->bv_right);
                $paidbv = $weak;

                $payment = User::with('plan')->lockForUpdate()->find($uex->user_id);
                ...
                app(LevelIncomeService::class)->distribute($payment, $trx, (float) $creditAmount);

                if ($isCapped) {
                    if ($consumedBv != 0) {
                        createBVLog($payment->id, 1, $consumedBv, 'Paid ' . $creditAmount . ' ' . $generalSetting->cur_text . ' for ' . $consumedBv . ' matched BV after capping.');
                        createBVLog($payment->id, 2, $consumedBv, 'Paid ' . $creditAmount . ' ' . $generalSetting->cur_text . ' for ' . $consumedBv . ' matched BV after capping.');
                    }
                    continue;
                }

                if ($paidbv != 0) {
                    createBVLog($payment->id, 1, $paidbv, 'Paid ' . $bonus . ' ' . $generalSetting->cur_text . ' For ' . $paidbv . ' matched BV.');
                    createBVLog($payment->id, 2, $paidbv, 'Paid ' . $bonus . ' ' . $generalSetting->cur_text . ' For ' . $paidbv . ' matched BV.');
                }
            }
