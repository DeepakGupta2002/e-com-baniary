                ...
                <div class="account-form-wrapper @if (!gs('registration')) form-disabled @endif">
                    ...
                    <form class="account-form verify-gcaptcha disableSubmission" method="POST" action="{{ route('user.register') }}">
                        ...
                        <div class="row">
                            @if ($refUser == null)
                                <div class="col-lg-6">
                                    <div class="form--group">
                                        <label class="form--label required">@lang('Referral Username')</label>
                                        <input class="form-control form--control" id="referenceBy" name="referBy" type="text" value="{{ old('referBy') }}"
                                            required placeholder="@lang('Enter Referral Username')">
                                        <small id="ref"></small>
                                    </div>
                                </div>
                            @else
                                ...
                                <div class="form--group">
                                    <label class="form--label required">@lang('Referral Username')</label>
                                    <input class="form-control form--control" id="referenceBy" name="referBy" type="text"
                                            value="{{ old('referBy', $refUser->username) }}" required readonly>
                                        <input id="referrer_id" name="referrer_id" type="hidden" value="{{ $refUser->id }}">
                                        <small id="ref"></small>
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="col-lg-6">
                                <div class="form--group">
                                    <label class="form--label required">@lang('Position')</label>
                                    <select class="form-control form--control" id="position" name="position" required>
                                        <option value="">@lang('Select Position')</option>
                                        <option value="{{ \App\Constants\Status::LEFT }}"
                                            @selected(old('position', $position) == \App\Constants\Status::LEFT)>
                                            @lang('Left')
                                        </option>
                                        <option value="{{ \App\Constants\Status::RIGHT }}"
                                            @selected(old('position', $position) == \App\Constants\Status::RIGHT)>
                                            @lang('Right')
                                        </option>
                                    </select>
                                    <small id="position-test"></small>
                                </div>
                            </div>
                        </div>
...
@push('script')
    <script>
        (function($) {
            "use strict";
            var not_select_msg = '@lang('Select referral username and position')';
            ...
            $('#referenceBy').on('blur', function(e) {
            ...
            function updateHand() {
                var url = '{{ route('get.user.position') }}';
                var referrer = $('#referrer_id').val();
                ...
                if (referrer && position) {
                    ...
                    $('#position-test').html('<span class="text-muted">' + not_select_msg + '</span>');
                }
            }
